...
def get_base_model(model, norm_func, in_channel=3):
    model = DLABase(levels=model["levels"],
                    channels=model["channels"],
                    block=eval(model["block"]),
                    norm_func=norm_func, in_channel=in_channel)
    return model
...
class DLA_T(nn.Module):
    def __init__(self,
                 cfg,
                 last_level=5,
                 out_channel=0):
        super(DLA_T, self).__init__()

        down_ratio = cfg.MODEL.TRANSFER.DOWN_RATIO*4
        assert down_ratio in [2, 4, 8, 16]

        self.first_level = int(np.log2(down_ratio))
        self.last_level = last_level

        base_name = _STAGE_SPECS[cfg.MODEL.TRANSFER.CONV_BODY]
        norm_func = _NORM_SPECS[cfg.MODEL.TRANSFER.USE_NORMALIZATION]
        channels = base_name['channels']

        # self.base = globals()[base_name]
        self.upsample = nn.Upsample(scale_factor=4, mode='bicubic', align_corners=True)
        self.base = get_base_model(base_name, norm_func, 128)
...
class DLABase(nn.Module):
    def __init__(self,
        ...
        self.base_layer = nn.Sequential(nn.Conv2d(in_channel,
                                                  channels[0],
                                                  kernel_size=7,
                                                  stride=1,
                                                  padding=3,
