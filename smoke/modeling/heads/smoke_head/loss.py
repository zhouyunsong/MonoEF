...
class SMOKELossComputation():
    ...
    def prepare_targets(self, targets):
        ...
        pitch_roll = torch.stack([t.get_field("pitch_roll") for t in targets])
    ...
    def prepare_predictions(self, targets_variables, pred_regression):
        ...
        pred_P_matrix = pred_regression_pois[-2:]
    ...
    def __call__(self, predictions, targets):
        ...
        if self.head_type == 'pitch_roll_head':
            ...
            pred_P_change = predictions[0]        
            target_pitch_roll = targets_variables["pitch_roll"]
            pitch_loss = F.l1_loss(pred_P_change[:,0], target_pitch_roll.float()[:,0], reduction="sum")/5
            roll_loss = F.l1_loss(pred_P_change[:,1], target_pitch_roll.float()[:,1], reduction="sum")/5

            return pitch_loss, roll_loss


def make_smoke_loss_evaluator(cfg, head_type='det_head'):
